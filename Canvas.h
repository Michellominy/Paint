...
public:
	...
	void drawBetween(int x1, int y1, int x2, int y2) {
        ...
        int firstDataIndex = getIndexOfWindowPos(x1, y1);
        glBufferSubData(GL_ARRAY_BUFFER, firstDataIndex * sizeof(Point), dx * sizeof(Point), &points[firstDataIndex]);
	}
        ...
        for (int i = 0; i <= dx; i++) {
            ...
            if (pk < 0) {
                if (decide == 0) {
                    ...
                    points[getIndexOfWindowPos(y1, x1)].color[0] = 1.0;
